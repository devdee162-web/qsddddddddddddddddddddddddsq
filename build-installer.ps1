# build-installer.ps1 — Build Zcord-Installer.exe (Electron Portable)
...
$OutExe = Join-Path $OutDir "Zcord-Installer.exe"

Write-Host ""
Write-Host "  [Zcord] Building Electron installer..." -ForegroundColor Cyan

# ── Fermeture des processus verrouilles ─────────────────────────────────────────
try { Stop-Process -Name "Zcord", "Zcord-Installer" -Force -ErrorAction SilentlyContinue } catch {}
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  Zcord-Installer.exe compile ($size MB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERREUR] Zcord-Installer.exe introuvable apres compilation." -ForegroundColor Red
    exit 1
}
