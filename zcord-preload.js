// Zcord preload — globalPaths fix + Equicord avec contextBridge
"use strict";
    ...
    try {
        ...
        if (rendererJs) {
            webFrame.executeJavaScript(rendererJs).catch(e => {
                console.error("[Zcord] renderer inject failed:", e?.message);
            });
        }
    } catch (e) {
        console.error("[Zcord] VencordPreloadGetRendererJs failed:", e);
    }
