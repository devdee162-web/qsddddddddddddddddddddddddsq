...
all: $(BUILD_DIR)/zcord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/zcord_rc.o: zcord.rc assets/zcord.ico
	mkdir -p $(BUILD_DIR)
	windres zcord.rc -o $@
...
$(BUILD_DIR)/zcord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/zcord.desktop
	rm -f $(PREFIX)/share/pixmaps/zcord.png
	rm -f $(PREFIX)/share/pixmaps/zcord.svg
	rm -f $(PREFIX)/bin/zcord

install: uninstall
	cp zcord.desktop $(PREFIX)/share/applications/zcord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/zcord.svg
	cp $(BUILD_DIR)/zcord $(PREFIX)/bin/zcord

run: all
	cp assets/zcord.gschema.xml $(BUILD_DIR)/assets/zcord.gschema.xml
