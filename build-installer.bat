@echo off
title Zcord Installer - Build
cd /d "%~dp0"

echo.
echo  ====================================
echo   Zcord Installer - Build (Electron)
echo  ====================================
echo.
...
taskkill /F /IM Zcord.exe /IM Zcord-Installer.exe >nul 2>&1
...
:: Packaging electron-builder -> Zcord-Installer.exe dans release/installer
echo.
...
:: Verification
if not exist "release\installer\Zcord-Installer.exe" (
    echo.
...
for %%F in ("release\installer\Zcord-Installer.exe") do (
    echo.
    echo  [OK] Build reussi !
    echo  Fichier : release\installer\Zcord-Installer.exe (%%~zF octets)
    echo.
)
