# ==============================================================================
#  Zcord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Zcord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Zcord dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          ZCORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Zcord-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Zcord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Zcord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$ZcordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Zcord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "zcord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'zcord-dist.zip' introuvable dans la release $version.`n           Contactez le support Zcord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "zcord-dist.zip"
    ...
    Write-OK "Zcord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Zcord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Zcord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Zcord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Zcord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez zcord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
