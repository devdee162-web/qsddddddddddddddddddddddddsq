...
import { openZcordModal } from "@zcordplugins/compactMode/ZcordModal";
    ...
    return (
        <Tooltip text={tooltip ?? ""} position={position} shouldShow={tooltip != null}>
            {({ onMouseEnter, onMouseLeave }) => (
                <Clickable
                    {...{ innerRef: ref } as any}
                    className={classes(HeaderBarClasses.clickable, "zcord-header-btn", className)}
...
try { _stealthActive = localStorage.getItem("Zcord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Zcord_stealthMode", "1") : localStorage.removeItem("Zcord_stealthMode"); } catch { }
}

const NON_REACT_SELECTORS = [
    "#zcord-titlebar-btn",
    "#zcord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("zcord-stealth"); else document.body?.classList.remove("zcord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("zcord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("zcord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("zcord-stealth-change"));
}
...
try { _compactActive = localStorage.getItem("Zcord_compactMode") === "1"; } catch { }
...
function persistCompact(v: boolean) {
    try { v ? localStorage.setItem("Zcord_compactMode", "1") : localStorage.removeItem("Zcord_compactMode"); } catch { }
}

export function syncCompactBodyClass() {
    try {
        const stored = localStorage.getItem("Zcord_compactMode");
    ...
    try {
        if (_compactActive) {
            document.body?.classList.add("zcord-compact");
        } else {
            document.body?.classList.remove("zcord-compact");
        }
    } catch { }
...
export function toggleCompactMode() {
    ...
    _notifyCompactChange();
    try { if (_compactActive) document.body?.classList.add("zcord-compact"); else document.body?.classList.remove("zcord-compact"); } catch { }
    return _compactActive;
}

if (_compactActive) {
    try { document.body?.classList.add("zcord-compact"); } catch { }
}

export const compactListeners = new Set<() => void>();
export function _notifyCompactChange() {
    compactListeners.forEach(fn => fn());
    window.dispatchEvent(new Event("zcord-compact-change"));
}
...
const ChevronMenuIcon = (props: any) => (
        ...
        <div className="nc-settings-popout">
            ...
            <div className="nc-settings-popout-row" onClick={() => toggleStealthMode()}>
                <div className="nc-settings-popout-row-info">
                    ...
                    <div className="nc-settings-popout-row-desc">Hide all Zcord UI elements</div>
                </div>
...
function CompactHeaderBarToggle() {
    ...
    return (
        <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <HeaderBarButton
                icon={GearIcon}
                tooltip="Zcord Settings"
                onClick={() => openZcordModal()}
            />
        </div>
    );
}
