...
export type BadgeSource = "vencord" | "equicord" | "zcord" | "globalbadges" | "illegalcord";
...
export async function loadOwnHiddenBadgeSources(userId: string) {
    ...
    try {
        const localData = localStorage.getItem("zcord_hidden_badges");
        if (localData) {
    ...
    try {
        ...
        if (result?.config?.settings?.hidden !== undefined) {
            ...
            localStorage.setItem("zcord_hidden_badges", JSON.stringify(hidden));
        }
    } catch (e) {
