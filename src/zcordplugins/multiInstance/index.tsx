...
const MI_TOKEN_CACHE_KEY = "zcord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("zcord-mi-ctx-root") ?? document.createElement("div");
        el.id = "zcord-mi-ctx-root";
                ...
                <div className="mi-list">
                    ...
                    {allAccounts.length === 0 ? (
                        <div className="mi-empty">
                            ...
                            {t("Use")} "<strong>{t("Switch Account")}</strong>" {t("in Discord")}.
                        </div>
                    ) : allAccounts.map(acc => {
...
export default definePlugin({
    ...
    async start() {
        ...
        addHeaderBarButton("zcord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("zcord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("zcord-mi-ctx-root");
        if (root) root.remove();
    },
});
