...
import { ZCORD_AVATAR_BASE64 } from "./avatarData";

export const ZCORD_USER_ID = "999999999999999999";
export const ZCORD_CHANNEL_ID = "999999999999999990";

const LS_LAST_SEEN = "zcord_official_dm_last_seen";
const LS_LAST_TS = "zcord_official_dm_last_ts";
...
const API_URLS = [
    "https://social.zcord.st/api/v1/timelines/public?local=true&limit=20",
    "https://social.zcord.st/api/v1/timelines/public?limit=20"
];
...
const zcordAvatarUrl = ZCORD_AVATAR_BASE64;

const createChannelRecordFromServer = findByCodeLazy(".GUILD_TEXT]", "fromServer)");
const createMessageRecord = findByCodeLazy(".createFromServer(", ".isBlockedForMessage", "messageReference:");

function replaceDiscordWithZcord(text: string): string {
    if (!text || typeof text !== "string") return text;
    return text
        .replace(/Official Discord Message/gi, "Official Zcord Message")
        .replace(/Message officiel de Discord/gi, "Message officiel de Zcord")
        .replace(/Team Discord/gi, "Team Zcord")
        .replace(/l'équipe Discord/gi, "l'équipe Zcord")
        .replace(/official Discord notifications/gi, "official Zcord notifications")
        .replace(/notifications officielles de Discord/gi, "notifications officielles de Zcord")
        .replace(/Discord ne vous demandera/gi, "Zcord ne vous demandera")
        .replace(/Discord ne te demandera/gi, "Zcord ne te demandera")
        .replace(/Discord will never ask/gi, "Zcord will never ask");
}

function isZcordContext(node: Node): boolean {
    if (SelectedChannelStore.getChannelId() === ZCORD_CHANNEL_ID) return true;
    ...
    while (curr) {
        if (curr.getAttribute) {
            ...
            if (href && (href.includes(ZCORD_CHANNEL_ID) || href.includes(ZCORD_USER_ID))) return true;
            const ariaLabel = curr.getAttribute("aria-label");
            if (ariaLabel && ariaLabel.toLowerCase().includes("zcord")) return true;
            const dataId = curr.getAttribute("data-list-item-id") || curr.getAttribute("data-channel-id");
            if (dataId && (dataId.includes(ZCORD_CHANNEL_ID) || dataId.includes(ZCORD_USER_ID))) return true;
        }
...
function patchTextNodes(node: Node) {
    ...
    if (!isZcordContext(node)) return;

    isUpdatingDom = true;
    try {
        if (node.nodeType === Node.TEXT_NODE) {
            if (node.nodeValue && /Discord/i.test(node.nodeValue)) {
                const newText = replaceDiscordWithZcord(node.nodeValue);
            ...
            ["aria-label", "title"].forEach(attr => {
                ...
                if (val && /Discord/i.test(val)) {
                    el.setAttribute(attr, replaceDiscordWithZcord(val));
                }
            });
            ...
            const fixImg = (img: Element) => {
                ...
                if (currentSrc && !currentSrc.startsWith("data:image")) {
                    if (currentSrc.includes(ZCORD_USER_ID) || currentSrc.includes("zcord_avatar") || currentSrc.includes("social.zcord.st/favicon")) {
                        if (img instanceof HTMLImageElement) img.src = zcordAvatarUrl;
                        img.setAttribute("href", zcordAvatarUrl);
                        img.setAttribute("xlink:href", zcordAvatarUrl);
                    }
                }
            };
...
function handleGlobalClick(e: MouseEvent) {
    if (SelectedChannelStore.getChannelId() !== ZCORD_CHANNEL_ID) return;
    ...
    if (btn) {
        ...
        if (txt === "learn more" || txt === "en savoir plus") {
            e.preventDefault();
            e.stopPropagation();
            window.open("https://zcord.st", "_blank");
        }
    }
}
...
const rawZcordUser = {
    ...
    getAvatarURL: () => zcordAvatarUrl,
    getAvatarSource: () => ({ uri: zcordAvatarUrl }),
    getAvatarDecorationURL: () => null,
    getBannerURL: () => null,
    toString: () => `<@${ZCORD_USER_ID}>`,
};

const rawZcordUserProxy = new Proxy(rawZcordUser, {
...
export const ZCORD_USER = proxyLazy(() => {
    try {
        ...
        if (URec) {
            ...
            u.getAvatarURL = () => zcordAvatarUrl;
            u.getAvatarSource = () => ({ uri: zcordAvatarUrl });
    ...
    return rawZcordUserProxy;
});

const rawZcordChannel = {
    id: ZCORD_CHANNEL_ID,
    type: 1, // DM
    name: "Zcord",
    guild_id: null,
    guildId: null,
    getGuildId: () => null,
    getRecipientId: () => ZCORD_USER_ID,
    recipients: [ZCORD_USER_ID],
    rawRecipients: [ZCORD_USER],
    isSystem: () => true,
    isOfficialSystem: () => true,
    isSystemDM: () => true,
    isDM: () => true,
...
const rawZcordChannelProxy = new Proxy(rawZcordChannel, {
    get(target, prop, receiver) {
        if (prop in target) return Reflect.get(target, prop, receiver);
        if (typeof prop === "string") {
            if (prop.startsWith("is") || prop.startsWith("has") || prop.startsWith("can")) {
...
export const ZCORD_CHANNEL = proxyLazy(() => {
    try {
        if (typeof createChannelRecordFromServer === "function") {
            ...
            if (ch) {
                ...
                ch.isSystemDM = () => true;
                ch.getRecipientId = () => ZCORD_USER_ID;
                return ch;
            }
        }
    } catch {}
    return rawZcordChannelProxy;
});
...
async function checkAndInjectPosts(isInitialLoad = false) {
    ...
    try {
        ...
        setTimeout(() => {
            ...
            if (newPostCount > 0) {
                if (!settings.store.isDmClosed && SelectedChannelStore.getChannelId() !== ZCORD_CHANNEL_ID) {
        ...
        console.warn("[ZcordOfficialDM] Error checking posts:", e);
    }
}
...
function handleChannelClose(e: any) {
    ...
    if (id === ZCORD_CHANNEL_ID || id === "999999999999999990") {
        settings.store.isDmClosed = true;
                unreadCount = 0;
        if (SelectedChannelStore.getChannelId() === ZCORD_CHANNEL_ID) {
            try {
                FluxDispatcher.dispatch({
                    type: "CHANNEL_SELECT",
                    channelId: null,
...
const ZcordProfileSections = ErrorBoundary.wrap(({ isSideBar }: { isSideBar: boolean }) => (
    <>
        <Section
        ...
        >
            <BaseText size="sm" color="text-normal">
                {formatDate(ZCORD_CREATED_AT)}
            </BaseText>
        </Section>
    </>
), { noop: true });

export function ZcordSystemBanner() {
    return (
        <div className="zcord-official-banner">
            <div className="zcord-official-banner-content">
                ...
                <div>
                    <div className="zcord-official-banner-title">
                        Ce salon est réservé aux notifications officielles de Zcord.
                    </div>
                    <div className="zcord-official-banner-sub">
                        Zcord ne te demandera jamais ton mot de passe ou ton token de compte.
                    </div>
                </div>
            </div>
            <button
                className="zcord-official-banner-btn"
                onClick={() => window.open("https://zcord.st", "_blank")}
            >
                En savoir plus
            </button>
        </div>
...
export default definePlugin({
    name: "ZcordOfficialDM",
    description: "Official Zcord System DM channel with verified badge and Mastodon feed integration",
    ...
    patches: [
        {
            ...
            replacement: {
                ...
                replace: "(Vencord.Api.ChatButtons._injectButtons($1,arguments[0]),$self.isZcordChannel(arguments[0])?$self.renderBanner():$&)"
            }
        },
        // Inject username + creation date in DM sidebar profile panel
        {
            ...
            replacement: {
                ...
                replace: "$1,$self.renderZcordProfileSections({userId:$2,isSideBar:true})"
            }
        },
        // Inject in profile modal
        {
            ...
            replacement: {
                ...
                replace: "$1,$self.renderZcordProfileSections({userId:$2,isSideBar:false}),"
            }
        },
        ...
        // Direct React Component patch for DM list — filters out Zcord DM when closed
        {
            find: '"dm-quick-launcher"===',
            replacement: {
                match: /privateChannelIds:([^,]+)(?=,listRef:)/,
                replace: "privateChannelIds:$self.filterZcordDm($1)"
            }
        }
    ],

    filterZcordDm(channelIds: any) {
        if (!Array.isArray(channelIds)) return channelIds;
        const closed = settings.store.isDmClosed;
        if (closed) {
            return channelIds.filter((id: string) => id !== ZCORD_CHANNEL_ID);
        }
        return channelIds;
    },

    renderZcordProfileSections({ userId, isSideBar }: { userId: string, isSideBar: boolean }) {
        if (userId !== ZCORD_USER_ID) return null;
        return <ZcordProfileSections isSideBar={isSideBar} />;
    },

    isZcordChannel(props: any) {
        return props?.channel?.id === ZCORD_CHANNEL_ID || SelectedChannelStore.getChannelId() === ZCORD_CHANNEL_ID;
    },

    renderBanner() {
        return <ZcordSystemBanner />;
    },
    ...
    start() {
        ...
        try {
            ...
            (FluxDispatcher as any).dispatch = function(action: any) {
                ...
                if (
                    ...
                    (action?.channel?.id === ZCORD_CHANNEL_ID || action?.channelId === ZCORD_CHANNEL_ID) &&
            ...
            setTimeout(() => {
                try {
                    FluxDispatcher.dispatch({
                        type: "CHANNEL_DELETE",
                        channelId: ZCORD_CHANNEL_ID,
                        channel: { id: ZCORD_CHANNEL_ID, type: 1 },
                    });
                } catch {}
                try { (findStore("ChannelStore") as any)?.emitChange(); } catch {}
                try { (findStore("PrivateChannelSortStore") as any)?.emitChange(); } catch {}
        ...
        waitFor(["fromTimestamp", "extractTimestamp"], (m: any) => {
            if (m && typeof m.extractTimestamp === "function" && !unpatchSnowflakeUtils) {
                ...
                m.extractTimestamp = function(snowflake: string) {
                    if (snowflake === ZCORD_USER_ID || snowflake === "999999999999999999") {
        ...
        waitFor(["useName", "getGlobalName"], (m: any) => {
            if (m && typeof m.formatForDisplay === "function") {
                ...
                m.formatForDisplay = function(user: any, ...args: any[]) {
                    ...
                    if (id === ZCORD_USER_ID || id === "999999999999999999") return "zcord";
        ...
        try {
            if (DisplayProfileUtils?.getDisplayProfile) {
                ...
                DisplayProfileUtils.getDisplayProfile = function(userId: string, ...args: any[]) {
                    if (userId === ZCORD_USER_ID || userId === "999999999999999999") {
            ...
            if (IconUtils) {
                if (typeof IconUtils.getUserAvatarURL === "function") {
                    ...
                    IconUtils.getUserAvatarURL = function(user: any, ...args: any[]) {
                        ...
                        if (id === ZCORD_USER_ID || id === "999999999999999999") return zcordAvatarUrl;
            ...
            if (URec?.prototype) {
                if (typeof URec.prototype.getAvatarURL === "function") {
                    ...
                    URec.prototype.getAvatarURL = function(this: any, ...args: any[]) {
                        if (this?.id === ZCORD_USER_ID || this?.id === "999999999999999999") return zcordAvatarUrl;
                ...
                if (typeof URec.prototype.getAvatarSource === "function") {
                    ...
                    URec.prototype.getAvatarSource = function(this: any, ...args: any[]) {
                        if (this?.id === ZCORD_USER_ID || this?.id === "999999999999999999") return { uri: zcordAvatarUrl };
        ...
        // Patch UserProfileStore to return Staff badge, blue banner and Zcord profile
        try {
            ...
            if (UserProfileStore) {
                ...
                if (typeof UserProfileStore.isFetchingProfile === "function") {
                    ...
                    UserProfileStore.isFetchingProfile = function(userId: string) {
                        if (userId === ZCORD_USER_ID) return false;
                ...
                if (typeof UserProfileStore.getUserProfileFetchStatus === "function") {
                    ...
                    UserProfileStore.getUserProfileFetchStatus = function(userId: string) {
                        if (userId === ZCORD_USER_ID) return "SUCCESS";
        ...
        // Patch i18n intl methods ONLY for Zcord channel
        try {
            if (i18n?.intl?.string) {
                ...
                i18n.intl.string = function(descriptor: any) {
                    ...
                    if (SelectedChannelStore.getChannelId() === ZCORD_CHANNEL_ID) {
                        if (typeof res === "string") return replaceDiscordWithZcord(res);
                    }
                    return res;
                };
            ...
            if (i18n?.intl?.format) {
                ...
                i18n.intl.format = function(descriptor: any, values: any) {
                    ...
                    if (SelectedChannelStore.getChannelId() === ZCORD_CHANNEL_ID) {
                        if (typeof res === "string") return replaceDiscordWithZcord(res);
                        if (Array.isArray(res)) return res.map(item => typeof item === "string" ? replaceDiscordWithZcord(item) : item);
                    }
                    return res;
                };
        ...
        // Patch ReadStateStore to show red ping badge for new unread Zcord posts
        try {
            ...
            if (ReadStateStore) {
                ...
                ReadStateStore.getMentionCount = function(channelId: string) {
                    if (channelId === ZCORD_CHANNEL_ID) return unreadCount;
                ...
                ReadStateStore.hasUnread = function(channelId: string) {
                    if (channelId === ZCORD_CHANNEL_ID) return unreadCount > 0;
        ...
        try {
            ...
            if (MessageActions?.fetchMessages) {
                ...
                MessageActions.fetchMessages = function(opts: any) {
                    if (opts?.channelId === ZCORD_CHANNEL_ID) {
                        checkAndInjectPosts(true);
                        return Promise.resolve();
                    }
            ...
            if (HTTP) {
                if (HTTP.get) {
                    ...
                    HTTP.get = function(opts: any) {
                        ...
                        if (url && (url.includes(ZCORD_CHANNEL_ID) || url.includes(ZCORD_USER_ID) || url.includes("999999999999999999"))) {
        ...
        UserStore.getUser = function(id: string) {
            if (id === ZCORD_USER_ID || id === "999999999999999999") return ZCORD_USER;
        ...
        if ((UserStore as any).getUsers) {
            ...
            (UserStore as any).getUsers = function() {
                ...
                if (!users[ZCORD_USER_ID]) {
                    return { ...users, [ZCORD_USER_ID]: ZCORD_USER };
                }
                return users;
            };
        ...
        try {
            ...
            actionMods.forEach((mod: any) => {
                ...
                closeMethods.forEach(methodName => {
                    if (typeof mod[methodName] === "function" && !mod[methodName].__zcordPatched) {
                        ...
                        mod[methodName] = function(...args: any[]) {
                            ...
                            if (targetId === ZCORD_CHANNEL_ID || targetId === "999999999999999990") {
                                settings.store.isDmClosed = true;
                                                                unreadCount = 0;
                                try {
                                    FluxDispatcher.dispatch({
                        ...
                        mod[methodName].__zcordPatched = true;
                    }
                });

                if (typeof mod.openPrivateChannel === "function" && !mod.openPrivateChannel.__zcordPatched) {
                    ...
                    mod.openPrivateChannel = function(...args: any[]) {
                        ...
                        if (targetId === ZCORD_USER_ID || targetId === "999999999999999999") {
                            settings.store.isDmClosed = false;
                                                        unreadCount = 0;
                            setTimeout(() => {
                                try {
                            ...
                            return Promise.resolve(ZCORD_CHANNEL_ID);
                        }
                        return origOpen.apply(this, args);
                    };
                    mod.openPrivateChannel.__zcordPatched = true;
                }
            });
        } catch {}
        ...
        ChannelStore.getChannel = function(id: string) {
            if (id === ZCORD_CHANNEL_ID) {
                // Always return the channel so navigation works (even when closed)
                // Visibility in the DM list sidebar is handled by filterZcordDm
                return ZCORD_CHANNEL;
            }
        ...
        if (channelStoreAny.getMutablePrivateChannels) {
            ...
            channelStoreAny.getMutablePrivateChannels = function() {
                ...
                const closed = settings.store.isDmClosed;
                if (!closed && !chs[ZCORD_CHANNEL_ID]) {
                    return { ...chs, [ZCORD_CHANNEL_ID]: ZCORD_CHANNEL };
                }
                if (closed && chs[ZCORD_CHANNEL_ID]) {
                    const copy = { ...chs };
                    delete copy[ZCORD_CHANNEL_ID];
                    return copy;
                }
                return chs;
            };
...
function getChannelTimestamp(channelId: string): number {
    ...
    return 0;
        ...
        const patchPrivateChannelSortStore = (SortStore: any) => {
            if (!SortStore || (SortStore as any).__zcordPatched) return;
            (SortStore as any).__zcordPatched = true;
            ...
            const patchSortFn = (fnName: string) => {
                if (typeof SortStore[fnName] === "function") {
                    ...
                    SortStore[fnName] = function(...args: any[]) {
                        ...
                        if (unreadCount > 0) {
                            ids.unshift(ZCORD_CHANNEL_ID);
                            return ids;
                        }

                        try {
                            const MessageStore = findStore("MessageStore") as any;
                            const zcordLastMsg = MessageStore?.getLastMessage?.(ZCORD_CHANNEL_ID);
                            let zcordTs = lastPostTimestamp;
                            if (zcordLastMsg?.timestamp) {
                                zcordTs = new Date(zcordLastMsg.timestamp).getTime();
                            }
                            ...
                            if (zcordTs > 0) {
                                ...
                                for (let i = 0; i < ids.length; i++) {
                                    ...
                                    if (zcordTs >= chTs) {
                                        ids.splice(i, 0, ZCORD_CHANNEL_ID);
                                        inserted = true;
                                        break;
                                    }
                                }
                                if (!inserted) {
                                    ids.push(ZCORD_CHANNEL_ID);
                                }
                            } else {
                                ids.push(ZCORD_CHANNEL_ID);
                            }
                    ...
                    sortUnpatches.push(() => {
                        ...
                        delete (SortStore as any).__zcordPatched;
                    });
                }
            };
        ...
        // Patch MessageRequestStore to explicitly exclude ZCORD_CHANNEL_ID from spam / message requests
        try {
            ...
            if (MsgReqStore) {
                if (typeof MsgReqStore.isSpam === "function") {
                    ...
                    MsgReqStore.isSpam = function(channelId: string) {
                        if (channelId === ZCORD_CHANNEL_ID) return false;
