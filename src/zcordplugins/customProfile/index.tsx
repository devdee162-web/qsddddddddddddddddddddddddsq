...
import { AuthenticationStore, Button, FluxDispatcher, GuildMemberStore, IconUtils, Menu, OAuth2AuthorizeModal, React, Select, SettingsRouter, showToast, SnowflakeUtils, Toasts, UserProfileStore, UserStore } from "@webpack/common";
...
const FAKE_PLATFORMS = [
    { id: "domain", label: "Domain / Site Web", icon: "/assets/b4376756bcbbf1ef.svg", placeholder: "zcord.st", defaultUrl: (name: string) => name.startsWith("http") ? name : `https://${name}` },
    { id: "twitter", label: "X (Twitter)", icon: "/assets/a61999ae9bfb9658.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://x.com/${name.replace(/^@/, "")}` },
    { id: "github", label: "GitHub", icon: "/assets/a35ff3e86ffa1eb2.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://github.com/${name}` },
    { id: "youtube", label: "YouTube", icon: "/assets/0fa530ba9c04ac32.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://youtube.com/@${name.replace(/^@/, "")}` },
    { id: "twitch", label: "Twitch", icon: "/assets/4fda00c96319c8ae.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://twitch.tv/${name}` },
    { id: "spotify", label: "Spotify", icon: "/assets/d5719388ffc613da.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://open.spotify.com/user/${name}` },
    { id: "tiktok", label: "TikTok", icon: "/assets/b4376756bcbbf1ef.svg", placeholder: "zcord", defaultUrl: (name: string) => name.startsWith("http") ? name : name.includes("tiktok.com") ? `https://${name.replace(/^https?:\/\//, "")}` : `https://tiktok.com/@${name.replace(/^@/, "")}` },
    { id: "reddit", label: "Reddit", icon: "/assets/adfd927dcc2049a5.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://reddit.com/user/${name}` },
    { id: "steam", label: "Steam", icon: "/assets/1f7ec18f3695d4cf.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://steamcommunity.com/id/${name}` },
    { id: "leagueoflegends", label: "Riot Games", icon: "https://cdn.discordapp.com/app-icons/1443033465766281327/b69039088ad141a9c036f7f4f247f6ba.png?size=128", placeholder: "Zcord#EUW" },
    { id: "bluesky", label: "Bluesky", icon: "/assets/2709f058378a099d.svg", placeholder: "zcord.bsky.social", defaultUrl: (name: string) => `https://bsky.app/profile/${name}` },
    { id: "paypal", label: "PayPal", icon: "/assets/dcb64a4ff8f61b2c.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://paypal.me/${name}` },
    { id: "ebay", label: "eBay", icon: "/assets/b28a7a265581b6e3.svg", placeholder: "zcord" },
    { id: "crunchyroll", label: "Crunchyroll", icon: "/assets/94ef3e8b7fa85a2e.svg", placeholder: "zcord" },
    { id: "playstation", label: "PlayStation Network", icon: "/assets/eed203a7ec517e23.svg", placeholder: "Zcord_ID" },
    { id: "xbox", label: "Xbox", icon: "/assets/c4f09fda61827e19.svg", placeholder: "Zcord" },
    { id: "amazon-music", label: "Amazon Music", icon: "/assets/e3c4aacc1a54395d.svg", placeholder: "zcord" },
    { id: "battlenet", label: "Battle.net", icon: "/assets/163c8cb9220efc74.svg", placeholder: "Zcord#1234" },
    { id: "bungie", label: "Bungie.net", icon: "/assets/099cd81cf3b4cc98.svg", placeholder: "Zcord#1234" },
    { id: "epicgames", label: "Epic Games", icon: "/assets/199eceff4fca1a0c.svg", placeholder: "Zcord" },
    { id: "facebook", label: "Facebook", icon: "/assets/17be29f77bee4405.svg", placeholder: "zcord", defaultUrl: (name: string) => `https://facebook.com/${name}` },
    { id: "roblox", label: "Roblox", icon: "/assets/a4d8e9b0404a2d00.svg", placeholder: "Zcord" }
];
...
const LS_KEY_DATA = "ZcordCP_data";
const LS_KEY_ENABLED = "ZcordCP_enabled";
const DS_ALL_DATA = "customProfile_allData";
const DS_ALL_ENABLED = "customProfile_allEnabled";
const LS_ALL_DATA = "ZcordCP_allData";
const LS_ALL_ENABLED = "ZcordCP_allEnabled";
...
async function fetchPublicProfileIfNeeded(userId: string) {
    ...
    if (inFlightRequests.has(userId) || inFlightRequests.size >= 4) return;
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={{
                ...
                <br />
                {t("Add social links or custom domains (e.g. zcord.st, X @tag, GitHub, Spotify, TikTok). These connections will appear directly on your Discord profile.")}
            </div>
            ...
            <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    <Field
                        label={platform === "domain" ? t("Domain / Website Name (e.g. zcord.st)") : t("Account Name / Username")}
                        value={name}
                        placeholder={currentPlat.placeholder || "zcord"}
                        onChange={setName}
                    />
                        ...
                        {activeTab === 'general' && (
                            <>
                                <Toggle
                                    ...
                                    sublabel={t("Lets other Zcord users see your Custom Profile, and lets you see theirs")}
                                ...
                                <div style={{
                                    ...
                                    <span style={{ color: "rgba(250, 166, 26, 0.9)", fontSize: 14, lineHeight: 1.5, fontWeight: 500 }}>
                                        {t("This requires Discord authorization. Once enabled, everyone using Zcord will be able to see your Custom Profile, and you will be able to see theirs.")}
                                    </span>
                                </div>
...
export default definePlugin({
    ...
    fakeCurrentUser(user: any) {
        try {
            if (!user || typeof user !== "object" || (!isEnabled && this._forceNative !== true) || !isMe(user.id)) return user;
        } catch {
            return user;
        }
    ...
    hookOtherUserProfile(profile: any, data: CustomProfileData, timestamp = 0) {
        ...
        try {
            ...
            badgesArr = badgesArr.filter(b => {
                ...
                if (id.includes("zcord") || desc.includes("zcord")) return false;
