...
// Discord settings crash: a boolean `true` ends up in the settings layout tree,
// then Discord calls methods on it (`.supports()`, `.useSetting()`, …).
try {
    const proto = Boolean.prototype as any;
 ...
 * zcord-index.js will apply the staged files on the next restart.
 */
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Zcord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Zcord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Zcord.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    createAndAppendStyle("zcord-titlebar-position", coreStyleRootNode).textContent = `
        body:not(.zcord-stealth):not(.zcord-compact) [class*="title_c38"] {
