...
export default {
    ...
    updater: {
        ...
        downloadAndRun: (url: string) => invoke<boolean>(IpcEvents.ZCORD_DOWNLOAD_AND_RUN, url),
    },
    ...
    zcord: {
        ...
        onUpdateStatus: (cb: (status: string, detail: string) => void) => {
            ipcRenderer.on(IpcEvents.ZCORD_UPDATE_STATUS, (_, status: string, detail: string) => cb(status, detail));
        },
    },
