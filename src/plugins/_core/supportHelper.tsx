...
import { platformName } from "@zcordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Zcord developers"
        const zcordGroupIds = [
            "1400111418358894646", "1172305545554825259", "407134577748869122",
            "587626543874834463", "1188391631662108752"
        ];

        const isOriginalDev = originalDevIds.includes(userId);
        const isZcordGroup = zcordGroupIds.includes(userId);

        if (!isOriginalDev && !isZcordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Zcord developers";
