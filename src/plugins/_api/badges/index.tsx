...
let ZcordBadges = {} as Record<string, Array<{ icon: string; placeholder: string; uuid: string; }>>;
...
async function loadAllBadges(noCache = false) {
    ...
    const zcordBadges = await loadBadges(`https://api.${domain}/badges`, noCache).catch(() => ({}));
...
export default definePlugin({
    ...
    getZcordBadges(userId: string) {
        try {
            const userBadges = ZcordBadges[userId];
            ...
            return userBadges
                ...
                .map(badge => ({
                    iconSrc: badge.icon,
                    description: badge.placeholder ?? "Zcord Badge",
                    position: BadgePosition.START,
                    props: {
                        style: {
                            borderRadius: "50%",
                    ...
                    onClick() {
                        return GenericBadgeModal(badge, "Zcord");
                    }
                } satisfies ProfileBadge));
        } catch (e) {
            console.error("[BadgeAPI] Error processing zcord badges for", userId, e);
            return [];
        }
    },
