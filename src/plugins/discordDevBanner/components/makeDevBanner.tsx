...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const zcordVersion = VERSION;
    const zcordHash = gitHashShort;
    const zcordPlatform = getVersionInfo(false);
    ...
    const replaced = baseFormat
        ...
        .replace(/{zcordVersion}|{equicordVersion}/g, zcordVersion)
        .replace(/{zcordHash}|{equicordHash}/g, zcordHash)
        .replace(/{zcordPlatform}|{equicordPlatform}/g, zcordPlatform)
