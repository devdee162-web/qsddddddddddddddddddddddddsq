...
const ZCORD_RELEASES_REPO = "zcord/zcord";
const ZCORD_REPO_URL = `https://source.${domain}/${ZCORD_RELEASES_REPO}`;
...
export { ZCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Zcord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(ZCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
