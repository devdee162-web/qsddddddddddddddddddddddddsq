...
export function PluginCard({ plugin, disabled, onRestartNeeded, onMouseEnter, onMouseLeave, isNew, hasTutorial }: PluginCardProps) {
    ...
    const openTutorialVideo = (e: React.MouseEvent) => {
        ...
        const videoUrl = `https://git.${domain}/zcord/zcord-tutorials/raw/branch/main/videos/${encodeURIComponent(videoName)}.mp4`;
        openModal(props => (
    ...
    const isZcord = !PluginMeta[plugin.name]?.userPlugin;
    const iconType = isZcord ? "zcord" : "other";

    // Le système de like ne s'applique qu'aux plugins Zcord (pas Vencord/Equicord,
    ...
    const isZcordFolderPlugin = PluginMeta[plugin.name]?.folderName?.startsWith("src/zcordplugins/") ?? false;
    const canShowLikeBadge = isZcordFolderPlugin && !plugin.required && !disabled;
        ...
        openModal(props => (
            <ModalRoot {...props} size={ModalSize.SMALL}>
                ...
                <ModalContent style={{ padding: "16px", display: "flex", flexDirection: "column", gap: "16px", alignItems: "center" } as any}>
                    {isZcord ? (
                        <a href="https://source.zcord.st/zcord/zcord" target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", gap: "12px", textDecoration: "none", color: "var(--text-normal)", fontSize: "20px", fontWeight: 600 }}>
                            <img src="https://source.zcord.st/assets/img/logo.svg" alt="Zcord" style={{ width: 64, height: 64, borderRadius: "50%" }} />
                            Zcord
                        </a>
                    ) : (
        ...
        <AddonCard
            ...
            infoButton={
                <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                    {(plugin.name === "DynamicIslande" || plugin.name === "StereoInstaller" || plugin.name === "ClientDiagnostics" || plugin.name === "SecureBookmarks" || plugin.name === "StatusCycler" || plugin.name === "MutualScanner") && (
                        <Tooltip text="This plugin comes from our partner (Illegalcord) then was modified by Zcord.">
                            {({ onMouseEnter, onMouseLeave }) => (
                                <button
                                    role="button"
