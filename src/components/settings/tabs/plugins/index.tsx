...
import { ZcordIcon } from "@components/Icons";
...
function ZcordTabIcon() {
    return <ZcordIcon width={18} height={18} />;
}
...
function UserPluginsTabIcon() {
    return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill="#e85d3a" d="M20.5 11H19V7c0-1.1-.9-2-2-2h-4V3.5C13 2.12 11.88 1 10.5 1S8 2.12 8 3.5V5H4c-1.1 0-1.99.9-1.99 2v3.8H3.5c1.49 0 2.7 1.21 2.7 2.7s-1.21 2.7-2.7 2.7H2V20c0 1.1.9 2 2 2h3.8v-1.5c0-1.49 1.21-2.7 2.7-2.7 1.49 0 2.7 1.21 2.7 2.7V22H17c1.1 0 2-.9 2-2v-4h1.5c1.38 0 2.5-1.12 2.5-2.5S21.88 11 20.5 11z" />
        </svg>
    );
}
...
function LikedPluginsTabIcon() {
    return (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path fill="#e85d3a" d="M12.47 21.73a.92.92 0 0 1-.94 0C9.43 20.48 1 15.09 1 8.75A5.75 5.75 0 0 1 6.75 3c2.34 0 3.88.9 5.25 2.26A6.98 6.98 0 0 1 17.25 3 5.75 5.75 0 0 1 23 8.75c0 6.34-8.42 11.73-10.53 12.98Z" />
        </svg>
    );
}
...
const makeCategoryOptions = (othersCount?: number) => [
    ...
    { label: "Zcord", value: SearchStatus.ZCORD },
...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Zcord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.ZCORD });
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.ZCORD:
                if (!pluginMeta?.folderName?.startsWith("src/zcordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (pluginMeta?.folderName?.startsWith("src/zcordplugins/") || pluginMeta?.folderName?.startsWith("src/plugins/_")) return false;
    ...
    // Only filter/categorize plugin DATA here — no JSX created yet
        const { zcordData, othersData, requiredData } = useMemo(() => {
        const zcordData: typeof sortedPlugins = [];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                ...
                if (folderName.startsWith("src/zcordplugins/")) {
                    zcordData.push(p);
                } else {
                    othersData.push(p);
                }
        ...
        zcordData.sort(byLikes);
        othersData.sort(byLikes);

        return { zcordData, othersData, requiredData };
    }, [sortedPlugins, searchValue, newPluginsSet, depMap, pluginFilter, ratings]);

    const allDataLength = zcordData.length + othersData.length;
    ...
    const zcordVisibleData = zcordData.slice(0, Math.min(visibleCount, zcordData.length));
    const othersVisibleData = othersData.slice(0, Math.max(0, visibleCount - zcordData.length));
    ...
    const makeRequiredCard = (p: typeof sortedPlugins[number]) => {
        const tooltipText = p.required || !depMap[p.name]
            ? "This plugin is required for Zcord to function."
    ...
    const zcordPlugins = zcordVisibleData.map(makeCard);
    const othersVisible = othersVisibleData.map(makeCard);
    const requiredPlugins = requiredData.map(makeRequiredCard);

    const totalZcordPlugins = React.useMemo(() => {
        return Object.values(Plugins).filter(p => PluginMeta[p.name]?.folderName?.startsWith("src/zcordplugins/")).length;
    }, []);

    const totalOtherPlugins = React.useMemo(() => {
        const isZcordPlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/zcordplugins/");
        const isCorePlugin = (p: string) => PluginMeta[p]?.folderName?.startsWith("src/plugins/_");
        return Object.values(Plugins).filter(p => !isZcordPlugin(p.name) && !isCorePlugin(p.name)).length;
    }, []);

    // Category-aware stats for the "ENABLED PLUGINS" card: reflects whichever tab
    // (ZCORD / OTHERS / all) is currently selected, instead of always being global.
    const categoryStats = useMemo(() => {
        ...
        if (searchValue.status === SearchStatus.ZCORD) {
            plugins = plugins.filter(isZcordPlugin);
        } else if (searchValue.status === SearchStatus.OTHERS) {
            plugins = plugins.filter(p => !isZcordPlugin(p));
            ...
            plugins = plugins.filter(p => isZcordPlugin(p) && ratings[p]?.likedByMe);
        }
        ...
        <SettingsTab>
                ...
                {!premiumOnly && (
                    <div className={cl("ecosystem-banner")}>
                        <div className={cl("ecosystem-banner-text")}>
                            ...
                            <Paragraph>{t("Manage your Zcord and community plugins here. Enable, disable, and configure them to your liking.")}</Paragraph>
                        </div>
                            ...
                            onClick={() => {
                                ...
                                if (native?.process?.env) {
                                    ...
                                    if (home) {
                                        ...
                                        const folderPath = isWindows ? `${home}\\Documents\\Zcord\\userplugins` : `${home}/Documents/Zcord/userplugins`;
                ...
                <div className={classes(Margins.bottom20, cl("filter-controls"))}>
                    ...
                    <div className={cl("filter-buttons")} style={{ minWidth: 220 }}>
                        <SafeSearchableSelect
                            ...
                            value={makeCategoryOptions(totalOtherPlugins).find(o => o.value === searchValue.status)?.value ?? SearchStatus.ZCORD}
                            onChange={(v: any) => {
                            ...
                            renderOptionPrefix={(o: any) => {
                                if (o?.value === SearchStatus.ZCORD) return <ZcordTabIcon />;
            ...
            {premiumOnly ? (
                <>
                    ...
                    {zcordData.length || othersData.length
                        ? (
                            <div className={cl("grid")}>
                                {[...zcordPlugins, ...othersVisible].length
                                    ? [...zcordPlugins, ...othersVisible]
                ...
                <>
                    {zcordData.length > 0 && searchValue.status === SearchStatus.ZCORD && (
                        <div className={cl("grid")}>
                            {zcordPlugins}
                        </div>
                    )}
                    ...
                    {searchValue.status === SearchStatus.USER_PLUGINS && (
                        <>
                            ...
                            {zcordPlugins.length > 0 || othersVisible.length > 0 ? (
                                <div className={cl("grid")}>
                                    {[...zcordPlugins, ...othersVisible]}
                                </div>
                            ) : (
                                <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                                    ...
                                    <div style={{ fontSize: 13 }}>{t("Add .tsx files to your")} <code>Documents/Zcord/userplugins/</code> {t("folder and rebuild with a local build.")}</div>
                                </div>
                            )}
                        </>
                    )}

                    {(searchValue.status !== SearchStatus.ZCORD && searchValue.status !== SearchStatus.OTHERS && searchValue.status !== SearchStatus.USER_PLUGINS) && (
                        <div className={cl("grid")}>
                            {zcordPlugins}
                            {othersVisible}
                        </div>
                    ...
                    {zcordData.length === 0 && othersData.length === 0 && searchValue.status === SearchStatus.LIKED_PLUGINS && (
                        <div style={{ textAlign: "center", padding: "48px 16px", color: "var(--text-muted)" }}>
                            ...
                            <div style={{ fontSize: 13 }}>{t("Like a Zcord plugin from its card to see it here.")}</div>
                        </div>
                    )}

                    {zcordData.length === 0 && othersData.length === 0 && searchValue.status !== SearchStatus.USER_PLUGINS && searchValue.status !== SearchStatus.LIKED_PLUGINS && (
                        <ExcludedPluginsList search={search} />
                    )}
