...
export const DATA_DIR = process.env.ZCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "ZcordData", suffix)
        : join(app.getPath("userData"), "..", "Zcord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Zcord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
