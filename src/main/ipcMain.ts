...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "zcord-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "zcord-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "zcord-wbs-"));
...
const OFFICIAL_UPDATE_URL = getOfficialUpdateUrl();

ipcMain.handle(IpcEvents.ZCORD_DOWNLOAD_AND_RUN, async (event, url: string) => {
    if (!validateSender(event)) throw new Error("Unauthorized IPC invocation");
    const targetUrl = url || OFFICIAL_UPDATE_URL;
    ...
    if (release.kind === "files") {
        const { applyFileUpdates } = await import("./updater/zcordRelease");
        const { needsRestart } = await applyFileUpdates(release.manifest, release.fileMap);
        if (needsRestart) {
            app.relaunch();
            app.exit(0);
        }
    ...
    const info = release.kind === "setup" ? release : {
        kind: "setup" as const,
        url: targetUrl,
        version: "",
    ...
    const tmpPath = path.join(os.tmpdir(), `ZcordUpdate-${Date.now()}.exe`);

    await downloadReleaseFile(info, tmpPath);
    await runSetupInstaller(tmpPath);
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async (event) => {
    ...
    try {
        ...
        console.error("[Zcord] VBCable install failed:", err);
