...
import { BaseText } from "@Zcord/types/components";
import { React, useRef, useState } from "@Zcord/types/webpack/common";

import { cl } from "./Settings";

function detectSource(json: any): "equicord" | "vencord" | "zcord" | "unknown" {
    ...
    // Plugins spécifiques à Zcord
    if (pluginNames.some(p => ["ZcordHelper", "equicordHelper"].includes(p))) return "zcord";
...
function cleanForZcord(json: any): any {
    ...
    // Supprimer les plugins propres à Equicord/Vencord qui n'existent pas dans Zcord
    ...
    async function processFile(file: File) {
        ...
        try {
            ...
            const sourceLabel =
                ...
                source === "zcord" ? "Zcord" : "inconnu";

            const cleaned = cleanForZcord(json);
            ...
            setStatus("success");
            setMessage(`✅ Settings ${sourceLabel} importés avec succès ! Redémarre Zcord pour appliquer.`);
        } catch (err: any) {
            setStatus("error");
    ...
    return (
        <div className={cl("category")}>
            ...
            <BaseText size="sm" style={{ marginBottom: "12px", opacity: 0.7 }}>
                Drag & drop your Equicord or Vencord backup JSON file here to import your settings into Zcord.
                Plugin-specific settings that don't exist in Zcord will be automatically removed.
            </BaseText>
