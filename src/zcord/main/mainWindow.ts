...
async function initSpellCheckLanguages(_win: BrowserWindow, languages?: string[]) {
    try {
        if (!languages?.length) {
            try {
                languages = await sendRendererCommand(IpcCommands.GET_LANGUAGES, undefined, 4000);
            } catch {
                languages = app.getPreferredSystemLanguages?.() || [app.getLocale() || "en-US"];
            }
        }
        ...
        console.warn("[Zcord] Failed to init spellcheck languages:", err);
    }
}
...
function buildBrowserWindowOptions(): BrowserWindowConstructorOptions {
    ...
    const options: BrowserWindowConstructorOptions = {
        show: Settings.store.enableSplashScreen !== true && !CommandLine.values["start-minimized"],
        backgroundColor,
        icon: getAppIconPath(),
        webPreferences: {
...
function getAppIconPath() {
    return process.platform === "win32"
        ? join(STATIC_DIR, "icon.ico")
        : join(STATIC_DIR, "icon.png");
}

function getAppNativeIcon() {
    const ico = join(STATIC_DIR, "icon.ico");
    const png = join(STATIC_DIR, "icon.png");
    try {
...
function applyAppIcon(win: BrowserWindow) {
    try {
        ...
        else win.setIcon(getAppIconPath());
    } catch { /* ignore */ }
}
...
function createMainWindow() {
    ...
    applyAppIcon(win);
    win.on("page-favicon-updated", () => applyAppIcon(win));
    win.webContents.on("did-finish-load", () => applyAppIcon(win));
    win.on("ready-to-show", () => applyAppIcon(win));
    // Discord/Electron can overwrite the taskbar icon after load; re-assert a few times.
    const iconTimers = [500, 1500, 3000, 6000].map(ms => setTimeout(() => applyAppIcon(win), ms));
    win.on("closed", () => iconTimers.forEach(clearTimeout));
    ...
    win.webContents.on("console-message", e => {
        ...
        if (!text) return;
        if (text.includes("[Zcord CrashHandler]") || text.includes("TypeError") || text.includes("is not a function")) {
            console.error("[renderer]", text);
        }
    });
    win.webContents.on("render-process-gone", (_e, details) => {
        console.error("[Zcord] renderer gone:", details.reason, details.exitCode);
    });
...
const runVencordMain = once(() => {
    try {
        require(VENCORD_DIR);
    } catch (err) {
        console.error("Failed to load Zcord main bundle from", VENCORD_DIR, err);
    }
});
