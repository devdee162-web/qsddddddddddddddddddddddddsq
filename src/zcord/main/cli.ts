...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.ZcordDir) {
        console.error("Cannot repair: using custom Zcord directory. Remove it in settings first.");
        process.exit(1);
    }
...
if (!IS_DISCORD_DESKTOP && !(globalThis as any).__zcordCliInit) {
    (globalThis as any).__zcordCliInit = true;
