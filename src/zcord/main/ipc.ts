...
handleSync(IpcEvents.GET_VENCORD_PRELOAD_SCRIPT, () => {
    try {
        return readFileSync(join(VENCORD_DIR, "preload.js"), "utf-8");
    } catch (err) {
        console.error("Failed to read Zcord preload", err);
        return "";
    }
});
...
// Handler pour VencordNative.zcord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.ZcordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { ZcordDir } = State.store;
    if (!ZcordDir) return;

    const stats = await stat(ZcordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(ZcordDir);
});
