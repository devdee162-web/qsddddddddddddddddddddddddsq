/*
 * Zcord â€” Installer via EquilotlCli
 * TÃ©lÃ©charge EquilotlCli.exe depuis les releases Equicord et le lance
 * avec les variables d'environnement pointant vers les fichiers Zcord.
 *
 * L'exe affiche une interface graphique permettant de choisir le Discord cible.
 *
 * Usage:
 *   pnpm inject    â†’ installe Zcord dans le Discord choisi
 *   pnpm uninject  â†’ dÃ©sinstalle Zcord du Discord choisi
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[Zcord] Installer already present, using local copy.");
        return outputFile;
    }

    console.log("[Zcord] Downloading installer (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: { "User-Agent": "Zcord (https://github.com/zcordfr/zcord)" }
    });
    ...
    console.log("[Zcord] Installer downloaded successfully!");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[Zcord] dist/desktop/patcher.js not found!\x1b[0m");
        ...
        for (const ver of versions) {
            ...
            if (existsSync(join(base, ver)) && !existsSync(appAsarPath) && !existsSync(backupPath)) {
                try {
                    ...
                    console.log(`[Zcord] Supprimé le dossier de mise à jour Discord incomplet : ${join(base, ver)}`);
                } catch (e) {
                    console.warn(`[Zcord] Impossible de supprimer ${join(base, ver)}: ${e.message}`);
                }
            }
        }
    }
...
function cleanOldZcord(isUninstall) {
    console.log("[Zcord] Cleaning previous installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            if (existsSync(appDirPath)) {
                ...
                try {
                    ...
                    if (existsSync(pkgFile)) {
                        ...
                        if (pkg.name === "zcord") shouldDelete = true;
                ...
                if (shouldDelete) {
                    ...
                    console.log(`[Zcord] Removed legacy app/ folder in ${resourcesDir}`);
                    cleanedAny = true;
                }
            }
            ...
            if (isUninstall && existsSync(backupPath)) {
                ...
                console.log(`[Zcord] Restored _app.asar -> app.asar in ${resourcesDir}`);
                cleanedAny = true;
            }

        } catch (e) {
            console.error(`[Zcord] Error cleaning ${resourcesDir}:`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[Zcord] Cleanup done.");
    } else {
        console.log("[Zcord] Nothing to clean.");
    }
}
        ...
        for (const ver of versions) {
            ...
            if (existsSync(backupPath)) {
                ...
                if (existsSync(updateExe)) {
                    console.log(`[Zcord] Launching ${channel}...`);
                    ...
                    if (existsSync(directExe)) {
                        console.log(`[Zcord] Launching ${channel} (direct)...`);
                        exec(`"${directExe}"`);
                    }
                }
...
cleanOldZcord(isUninstall);
if (!isUninstall) checkBuild();

const installerBin = await ensureBinary();

console.log("[Zcord] Injecting...");
...
try {
    execFileSync(installerBin, mappedArgs, {
        ...
        env: {
            ...
            ZCORD_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[Zcord] Injection failed.");
    process.exit(1);
}
