...
const targetPlugins = [
    "FakeVoice", "antiMoveDeco", "zcordAI", "ClearGroups", "ClearDMs", "messageCleaner",
    ...
    "stereoInstaller.desktop", "translate", "fakeDM", "fakeAccount",
    "soundcloudPlayer", "youtubePlayer", "exportDM", "bulkFriendRemove",
    "sharePerms", "voiceChannelSearch", "wordBomb", "multiInstance", "customProfile",
    "autoCorrect", "autoReply", "autoResponder", "encryptedMessage",
    "gifConvertor", "selfDestruct", "streamProof", "voiceDictation"
];
...
function isRealText(str) {
    ...
    if (str === "Zcord" || str === "Zcord AI") return false;
...
for (const pluginFolder of targetPlugins) {
    const pluginPath = join("src/zcordplugins", pluginFolder);
    ...
    for (const file of files) {
        ...
        if (validRawJSX.length > 0) {
            // Check import
            if (!content.includes('autoTranslateZcord')) {
                // Find relative path to autoTranslateZcord
                const depth = file.split(/[/\\]/).length - file.split(/[/\\]/).indexOf("zcordplugins") - 2;
                const relPath = "../".repeat(Math.max(1, depth)) + "autoTranslateZcord";
