/*
 * Zcord — Local injector for Discord Desktop
 * Injecte Zcord dans une installation Discord existante en :
 * 1. Trouvant le répertoire resources de Discord
 * 2. Renommant app.asar → _app.asar (backup)
 * 3. Créant un dossier app/ avec un loader qui require le patcher.js de Zcord
 *
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[Zcord] dist/desktop/patcher.js introuvable !\x1b[0m");
...
function inject(resourcesDir) {
    ...
    if (existsSync(appDirPath) && existsSync(join(appDirPath, "index.js"))) {
        try {
            ...
            if (indexContent.includes("Zcord Injector") || indexContent.includes("Zcord")) {
                console.log("\x1b[33m[Zcord] Déjà injecté ! Utilisez 'pnpm uninject' d'abord pour réinjecter.\x1b[0m");
                return false;
            }
        } catch { }
    }
    ...
    if (existsSync(appAsarPath) && !existsSync(backupPath)) {
        ...
        if (isDir) {
            console.warn("\x1b[33m[Zcord] Nettoyage de l'ancien dossier app.asar...\x1b[0m");
            try { rmSync(appAsarPath, { recursive: true, force: true }); } catch { }
        } else {
            console.log("[Zcord] Sauvegarde app.asar → _app.asar...");
            renameSync(appAsarPath, backupPath);
        }
    } else if (!existsSync(backupPath)) {
        console.error("\x1b[31m[Zcord] Aucun app.asar ou _app.asar trouvé dans resources !\x1b[0m");
        return false;
    }
    ...
    if (existsSync(appAsarPath)) {
        try {
            ...
            console.error(`\x1b[31m[Zcord] Impossible de supprimer l'ancien app.asar : ${e.message}\x1b[0m`);
            return false;
        }
    }
    ...
    // Le loader require simplement le patcher Zcord depuis dist/
    ...
    console.log(`\x1b[32m[Zcord] Injecté avec succès dans : ${resourcesDir}\x1b[0m`);
    console.log(`\x1b[32m[Zcord] Répertoire Zcord dist : ${DIST_DIR}\x1b[0m`);
    console.log("\x1b[36m[Zcord] Redémarrez Discord pour appliquer les changements.\x1b[0m");
    return true;
}
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Zcord] Aucune installation Discord trouvée !\x1b[0m");
...
if (allResources.length === 1) {
    ...
    console.log(`[Zcord] Discord trouvé : ${allResources[0]}`);
    inject(allResources[0]);
} else {
    // Plusieurs Discord trouvés : injecter dans tous
    console.log(`[Zcord] ${allResources.length} installations Discord trouvées :`);
    let injectedCount = 0;
    for (const res of allResources) {
        console.log(`\n  → ${res}`);
        if (inject(res)) injectedCount++;
    }
    console.log(`\n\x1b[32m[Zcord] ${injectedCount}/${allResources.length} injection(s) réussie(s).\x1b[0m`);
}
