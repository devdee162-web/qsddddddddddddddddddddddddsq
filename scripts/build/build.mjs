...
const globNativesPlugin = {
    ...
    setup: build => {
        ...
        build.onLoad({ filter, namespace: "import-natives" }, async () => {
            const pluginDirs = ["plugins", "userplugins", "zcordplugins"];
            let code = "";
            let natives = "\n";
            let i = 0;
            /**
...
const buildConfigs = ([
    ...
    },
    {
        ...commonOpts,
        entryPoints: [join(dirname(fileURLToPath(import.meta.url)), "../../src/Vencord.ts")],
        outfile: "dist/zcord/renderer.js",
        format: "iife",
        target: ["esnext"],
...
await Promise.all([
    ...
    writeFile("dist/zcord/package.json", JSON.stringify({
        name: "zcord",
        main: "main.js"
    }))
]);
...
await Promise.all([
    ...
    createPackage("dist/zcord", "dist/zcord.asar"),
]);
