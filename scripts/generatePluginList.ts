...
(async () => {
    ...
    // Setup userplugins
    try {
        const userTarget = join(homedir(), "Documents", "Zcord", "userplugins");
        ...
        console.error("[Zcord] Failed to setup userplugins link", e);
    }
    ...
    const zcordFlag = args.includes("--zcord");

    let dirs: string[];

    if (zcordFlag) {
        dirs = ["src/zcordplugins"];
    } else if (equicordFlag) {
        dirs = ["src/zcordplugins", "src/userplugins"];
    } else if (vencordFlag) {
        dirs = ["src/plugins", "src/plugins/_core"];
    } else {
        dirs = ["src/plugins", "src/plugins/_core", "src/zcordplugins", "src/userplugins"];
    }
