...
const pluginsDir = path.join(process.cwd(), 'src', 'zcordplugins');
...
for (const p of pluginDirs) {
    ...
    if (nameMatch && descMatch) {
        ...
        // Add some specific instructions based on common Zcord plugin features
...
// Also let's handle Vencord plugins by just providing a fallback in the component, but we can do it here for zcord plugins.
