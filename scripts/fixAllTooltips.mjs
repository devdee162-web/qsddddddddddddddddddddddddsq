...
const files = getFiles("src/zcordplugins");
const tooltipsFound = [];
...
for (const file of files) {
    ...
    for (const m of tooltipMatches) {
        const attr = m[1];
        const val = m[2].trim();
        if (val && val !== "Zcord" && val !== "Zcord AI" && !val.startsWith("http") && !/^\d+$/.test(val)) {
            tooltipsFound.push({ file, attr, val });
            
            // Check if file has t imported
            if (!content.includes('autoTranslateZcord') && !content.includes('import { t }') && !content.includes('import { tPlugin as t }')) {
