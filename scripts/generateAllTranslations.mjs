...
// Read current autoTranslateZcord index.ts
const atcContent = readFileSync("src/zcordplugins/autoTranslateZcord/index.ts", "utf8");

// Parse existing entries in autoTranslateZcord
...
console.log(`Existing entries in autoTranslateZcord: ${Object.keys(existingEntries).length}`);

// Find strings that are not in existingEntries or have missing/corrupt translations
const missingStrings = strings.filter(s => {
    if (s === "Zcord" || s === "Zcord AI") return false;
...
async function main() {
    ...
    // Format new autoTranslateZcord index.ts
    let newContent = `/*
...
export default definePlugin({
    name: "AutoTranslateZcord",
    enabledByDefault: true,
    required: true,
    description: "Automatic translation for Zcord.",
...
`;
    ...
    writeFileSync("src/zcordplugins/autoTranslateZcord/index.ts", newContent, "utf8");
    console.log("Successfully updated autoTranslateZcord/index.ts!");
}
