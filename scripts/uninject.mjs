/*
 * Zcord — Local un-injector for Discord Desktop
...
function findAllDiscordResources() {
    ...
    // Filtrer uniquement les paths avec une injection Zcord présente
...
function uninject(resourcesDir) {
    ...
    // Vérifier que le dossier app/ a bien été créé par Zcord
    if (existsSync(appDirPath)) {
        try {
            if (existsSync(join(appDirPath, "index.js"))) {
                ...
                if (!indexContent.includes("Zcord Injector") && !indexContent.includes("Zcord")) {
                    console.warn(`\x1b[33m[Zcord] Le dossier app/ existe mais n'a pas l'air d'avoir été créé par Zcord.\x1b[0m`);
        ...
        console.log("[Zcord] Suppression du dossier app/ injecté...");
        rmSync(appDirPath, { recursive: true, force: true });
    } else {
        console.log("\x1b[33m[Zcord] Aucun dossier app/ injecté trouvé.\x1b[0m");
    }

    // Restaurer le backup
    if (existsSync(backupPath) && !existsSync(appAsarPath)) {
        console.log("[Zcord] Restauration _app.asar → app.asar...");
        renameSync(backupPath, appAsarPath);
    } else if (existsSync(backupPath) && existsSync(appAsarPath)) {
        console.log("[Zcord] app.asar déjà présent, nettoyage du backup...");
        rmSync(backupPath, { force: true });
    }

    console.log(`\x1b[32m[Zcord] Désinjection réussie depuis : ${resourcesDir}\x1b[0m`);
...
if (allResources.length === 0) {
    console.error("\x1b[31m[Zcord] Aucune installation Discord avec Zcord injecté trouvée.\x1b[0m");
    console.error("\x1b[33m           Assurez-vous que Zcord a bien été injecté via 'pnpm inject'.\x1b[0m");
    process.exit(1);
}
...
if (uninjectCount === 0) {
    console.error("\x1b[31m[Zcord] Aucune désinjection réussie.\x1b[0m");
    process.exit(1);
}

console.log(`\n\x1b[32m[Zcord] ${uninjectCount}/${allResources.length} désinjection(s) réussie(s).\x1b[0m`);
