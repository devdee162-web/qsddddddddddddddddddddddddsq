@echo off
:: ─── Zcord — Publier une nouvelle release sur Gitea ──────────────────────
...
if "%NOTES%"=="" set NOTES=Zcord %VERSION%

:: ── Config Gitea ──────────────────────────────────────────────────────────────
set GITEA_URL=https://source.zcord.st
set GITEA_REPO=zcord/zcord
...
set DIST_ZIP=%OUT_DIR%\zcord-dist.zip
set INSTALLER_EXE=%OUT_DIR%\Zcord-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    ZCORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Zcord-Installer.exe ────────────────────────────────────────────────
echo.
echo  [5/8] Compilation de Zcord-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] Zcord-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)
for %%F in ("%INSTALLER_EXE%") do echo  [5/8] Zcord-Installer.exe cree (%%~zF octets)

:: ── 6. zcord-dist.zip ─────────────────────────────────────────────────────
echo.
echo  [6/8] Creation de zcord-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'zcord-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"
if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer zcord-dist.zip
    pause
    exit /b 1
)
for %%F in ("%DIST_ZIP%") do echo  [6/8] zcord-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "%GITEA_URL%/%GITEA_REPO%/releases/download/v%VERSION%/Zcord-Installer.exe",
    echo   "distUrl": "%GITEA_URL%/%GITEA_REPO%/releases/download/v%VERSION%/zcord-dist.zip",
...
:: 8c. Upload — Zcord-Installer.exe (via curl, < 100MB)
echo  Upload de Zcord-Installer.exe...
curl -s -X POST "%GITEA_API%/repos/%GITEA_REPO%/releases/%RELEASE_ID%/assets?name=Zcord-Installer.exe" ^
    -H "Authorization: token %GITEA_TOKEN%" ^
    -H "Content-Type: application/octet-stream" ^
    --data-binary "@%INSTALLER_EXE%" >nul
if errorlevel 1 ( echo  [ERREUR] Upload Zcord-Installer.exe echoue. & pause & exit /b 1 )

:: 8d. Upload — zcord-dist.zip (via curl, < 100MB)
echo  Upload de zcord-dist.zip...
curl -s -X POST "%GITEA_API%/repos/%GITEA_REPO%/releases/%RELEASE_ID%/assets?name=zcord-dist.zip" ^
    -H "Authorization: token %GITEA_TOKEN%" ^
    -H "Content-Type: application/zip" ^
    --data-binary "@%DIST_ZIP%" >nul
if errorlevel 1 ( echo  [ERREUR] Upload zcord-dist.zip echoue. & pause & exit /b 1 )
...
powershell -NoProfile -Command ^
    ...
    "$uri = 'https://source.zcord.st/api/v1/repos/zcord/zcord/releases/%RELEASE_ID%/assets?name=desktop.asar';" ^
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════════════╗
echo  ║  Zcord v%VERSION% publie avec succes sur Gitea !
echo  ║
echo  ║  URL : %GITEA_URL%/%GITEA_REPO%/releases/tag/v%VERSION%
echo  ║
echo  ║  Fichiers publies :
echo  ║    Zcord-Installer.exe    — installeur .exe avec GUI
echo  ║    zcord-dist.zip         — JS obfusques (pour l'injec.)
