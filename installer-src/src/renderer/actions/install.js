...
const RELEASE_API = `https://source.${domain}/api/v1/repos/zcord/zcord/releases/latest`;
const DIST_ZIP = "zcord-dist.zip";
const distDir = path.join(process.env.LOCALAPPDATA, "Zcord", "dist");
...
async function cleanModulePatches(resourcesPath) {
    try {
        ...
        log(`[Zcord] CleanModulePatches warning: ${err.message}`);
    }
}
...
function downloadFileAsync(url, destPath, onProgress) {
    return new Promise((resolve, reject) => {
        ...
        https.get(url, { headers: { "User-Agent": "Zcord-Installer/3.0" }, rejectUnauthorized: false }, (response) => {
...
const getJSON = phin.defaults({
    ...
    headers: { "User-Agent": "Zcord-Installer/3.0", "Accept": "application/json" }
});
...
async function downloadDist() {
    ...
    try {
        ...
        zcordVersion = release && release.tag_name;
        if (!assetUrl) {
    ...
    const tmpZip = path.join(remote.app.getPath("temp"), "zcord-dist.zip");
    log(`Downloading Zcord ${zcordVersion} package...`);
    try {
        await downloadFileAsync(assetUrl, tmpZip, (percent, downloaded, total) => {
            ...
            status.set(`Downloading Zcord... (${dlMB}/${totalMB} MB)`);
        });
...
async function writeLoader(appDir) {
    ...
    const loaderCode = `// Zcord Injector
"use strict";
const fs = require('fs');
...
if (!fs.existsSync(patcherPath)) throw new Error('[Zcord] patcher.js not found. Expected at: ' + primary);
require(patcherPath);
`;
...
async function applyDefaultPluginsSetting() {
    try {
        const settingsDir = path.join(process.env.APPDATA, "Zcord", "settings");
        const settingsPath = path.join(settingsDir, "settings.json");
        await fs.mkdir(settingsDir, { recursive: true });

        // Always enable default plugins by removing the 'plugins' key from settings.json
        // so that Zcord natively loads its enabledByDefault values.
        let existing = null;
...
async function injectShims(paths) {
    ...
    for (const resPath of paths) {
        ...
        try {
            ...
            log("2. Configuring Zcord loader...");
...
export default async function(paths) {
    try {
        ...
        await fs.mkdir(path.join(localAppData, "Zcord"), { recursive: true });
        log("✅ Local AppData directory prepared");
        progress.set(MAKE_DIR_PROGRESS);
        lognewline("Downloading Zcord package...");
        ...
        lognewline("Injecting Zcord shims...");
