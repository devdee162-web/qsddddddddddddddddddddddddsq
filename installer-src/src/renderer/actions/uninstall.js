...
async function shouldAutoRestart() {
    try {
        const prefsPath = path.join(process.env.APPDATA, "Zcord", "settings", "installer-prefs.json");
...
async function deleteShims(paths) {
    ...
    for (const resPath of paths) {
        log(`Removing Zcord from: ${resPath}`);
        try {
            ...
            log(`❌ Could not remove Zcord from ${resPath}`);
            log(`❌ ${err.message}`);
            return err;
        }
    }
...
export default async function(paths) {
    try {
        ...
        lognewline("Deleting Zcord loader and restoring files...");
