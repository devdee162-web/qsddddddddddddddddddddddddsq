...
const getJSON = phin.defaults({
    method: "GET",
    parse: "json",
    headers: {"User-Agent": `Zcord Installer/${version}`},
    followRedirects: true
});

/* eslint-disable no-console */
export default async function () {
    const downloadUrl = `https://source.${domain}/api/v1/repos/zcord/zcord/releases/latest`;
    console.info(`Zcord Installer ${version}`);
    ...
    try {
        ...
        if (semverGreaterThan(latestVersion, version)) {
            ...
            const result = await dialog.showMessageBox({
                ...
                message: `A new version of the Zcord installer is available. Click "Download" to download the newest version.`,
